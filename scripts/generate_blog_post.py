#!/usr/bin/env python3
"""Generate one AvaLimo blog post and prepend it to public/blog_posts.json.

Idempotent per date: re-running for an existing date is a no-op, so daily
cron + backfill loops can safely overlap. Stdlib only.

Usage:
    python3 scripts/generate_blog_post.py [--date YYYY-MM-DD]

Env:
    OLLAMA_URL     Ollama chat endpoint (default: VPS shared endpoint)
    OLLAMA_MODELS  Comma-separated model fallback chain
"""
...
BLOG_FILE = os.path.join(REPO_ROOT, "public", "blog_posts.json")
...
def main() -> None:
    ...
    posts.insert(0, new)
    posts = sorted(posts, key=lambda p: str(p.get("date", "")), reverse=True)

    fd, temporary_path = tempfile.mkstemp(prefix="blog-posts-", suffix=".json", dir=os.path.dirname(BLOG_FILE))
    try:
